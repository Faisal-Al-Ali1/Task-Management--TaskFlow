...
const ArticleCard = ({ article, onClick }) => (
  <article
    className="group flex flex-col h-full bg-white rounded-3xl shadow-md transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 overflow-hidden cursor-pointer"
    onClick={onClick}
  >
    ...
    <div className="p-6 flex flex-col flex-1">
      ...
      <button
        className="mt-6 w-full px-6 py-3.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-medium transition-all duration-300 hover:from-indigo-700 hover:to-purple-700 hover:shadow-lg hover:shadow-indigo-500/25 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 flex items-center justify-center gap-2"
        onClick={(e) => {
          e.stopPropagation();
          onClick();
        }}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center max-w-md">
          ...
          <p className="text-gray-600">لا توجد مقالات متاحة.</p>
        </div>
      </div>
    );
